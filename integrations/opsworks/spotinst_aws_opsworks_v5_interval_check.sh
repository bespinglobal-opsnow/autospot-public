...
function validate {
  if [ -z $OPSWORKS_STACK_ID ] || [ -z $OPSWORKS_LAYER_ID ] || [ -z $OPSWORKS_STACK_TYPE ]; then
    log_error "OPSWORKS_STACK_TYPE, OPSWORKS_STACK_ID and OPSWORKS_LAYER_ID are required"
    exit 1
  fi

  if [ -z $WAIT_AFTER_REGISTRATION ]; then
    WAIT_AFTER_REGISTRATION=150
  fi
}
...
function install_opsworks {
  ...
  aws opsworks register --infrastructure-class ec2 --use-instance-profile --region $OPSWORKS_ENDPOINT --stack-id $OPSWORKS_STACK_ID --local --override-hostname $(curl -s http://169.254.169.254/latest/meta-data/instance-id)
  ...
  log_info "Poll every 15 seconds until a successfull state has been reached"
  aws opsworks wait instance-registered --region $OPSWORKS_ENDPOINT --instance-ids $instanceOpsworksId
  if [ $? == 0 ]; then
    ...
    log_info "Poll every 15 seconds until the instance becomes online"
    aws opsworks wait instance-online --region $OPSWORKS_ENDPOINT --instance-ids $instanceOpsworksId
    if [ $? == 0 ]; then
      log_info "Instance successfully assigned & online"
      log_info "Instance successfully registered and assigned to AWS OpsWorks; DONE"
    else 
      log_info "Instance failed to be assinged"
    fi
  else 
    log_info "Instance registrationn has failed"
  fi
}
